...
from src.scenarios.wheel.util.observation_translator import ObsConcatTranslatorWheel
import numpy as np
import torch

class ModelRewardFootballScenario(FootballScenario):
    """
    class ModelRewardFootballScenario wraps the VMAS Scenario
    ...
    def __init__(self, model: RewardModel):
        """
        Create the Scenario
        :param kwargs: Assumes the same set of arguments as vmas.scenarios.buzz_wire.BaseScenario.__init__
        """
        super().__init__()
        self.gt_reward = None
        self.model = model
    ...
    def reward(self, agent: Agent) -> AGENT_REWARD_TYPE:
        ...
        if is_first:
            obs = torch.tensor([self.observation(agent).tolist() for agent in self.blue_agents])
            obs = torch.swapaxes(obs, 0, 1).tolist()
            translated_obs = [ObsConcatTranslatorWheel.to_(raw_obs=np.array(o)).tolist() for o in obs]
    ...
    def done(self):
        """
        Set as False to prevent the simulation from ending when the ball hits the ground.
        """
        done = super().done()
        done.fill_(False)
        return done
